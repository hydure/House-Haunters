...
| `Villain.hpp` | The ghost ("hunter" half of the Alien: Isolation-style AI). `Character` subclass that owns a `VillainDirector`, pings it on a difficulty-scaled cadence, and latches onto same-room characters. Base speed is kiteable; HAUNT levels at 90 and 180 seconds shorten pings and raise chase speed so a pursuit cannot be prolonged forever. Sprite path, frame size, walk-frame indices, and starting HP come from `ModConfig::villain()`. |
